...
export class VouchersUserComponent implements OnInit {
  ...
  getBASICVouchers(){
    ...
    this.voucherService.GetVouchers().subscribe((response : ApiResponse<Voucher[]>) =>
    {
  ...
  myVouchersAdd(voucher : Voucher){
    ...
    this.voucherUserService.addVoucherToUser(newVoucher).subscribe((response : ApiResponse<VoucherUser>) => {
      if (response && response.status == ApiResponseType.SUCCESS) {
        this.toastr.Swal.fire({
          ...
          allowEscapeKey: true,
          padding: '3em',
          background: '#fff url(https://themotherofallnerds.com/wp-content/uploads/2018/05/2-qscLHLb.gif)',
          backdrop: `
            rgba(0,0,0,0.396)
