...
export class PersonalBookService {
  ...
  Mailme(mail : string,book:number){

    let params=new HttpParams()
    .set("book",book.toString())
    .append("mail",mail.toString());
    return this.httpClient.post<ApiResponse<Mailme>>(this.globalVarService.globalUrl + "/api/mailme",null , {params : params});
